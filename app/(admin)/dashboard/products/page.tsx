...
export default function ProductsListPage() {
  ...
  const key = `/api/admin/products/list?q=${encodeURIComponent(
    q
  )}&variant=${variant}&take=${PAGE_SIZE}&skip=${skip}`;
              ...
              <TableRow key={p.id} className="hover:bg-neutral-800/50">
                ...
                <TableCell className="text-right">
                  <div className="flex justify-end gap-2">
                    ...
                    >
                      <Link href={`/dashboard/products/${p.id.toString()}`}>
                        Editar
                      </Link>
                    </Button>
                    <Button
                      ...
                      onClick={async () => {
                        ...
                        const res = await fetch(
                          `/api/admin/products/${p.id.toString()}`,
                          { method: "DELETE" }
                        );
                        if (res.ok) {
